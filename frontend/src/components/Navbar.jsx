        ...
        <ul className="hidden sm:flex gap-5 text-sm text-gray-700">
                ...
                <div className='group relative'>
                 <Link to='/login'> <img className='w-5 cursor-pointer' src={assets.profile_icon} alt="" /> </Link>
                  ...
                  <Link to='/cart' className='relative'>
                    ...
                    <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading bg-black text-white aspect-square rounded-full text-[8px]'>{getCartCount()}</p>
                  </Link>
          ...
          <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ?"w-full" : "w-0"}`}>
              <div className='flex flex-col text-gray-600'>
                <div onClick={() => setVisible(false)} className='flex items-center ap-4 p-3 cursor-pointer'>
