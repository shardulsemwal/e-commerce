...
const removeProduct = async (id) => {
  ...
  return (
    ...
    <div className='flex flex-col gap-2'>
      <div className='hidden md:grid grid-cols-[1fr_3fr_1fr_1fr_1fr] bg-gray-100 px-2 py-2 items-center text-sm'>
        <b>Image</b>
        <b>Name</b>
        <b>Category</b>
        <b>Price</b>
      ...
      list.map((item,index) => (
        <div key={index} className='grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center gap-2 py-1 px-2 border text-sm'>
          <img className='w-12' src={getImageSrc(item.image)} alt={item.name} />
          <p>{item.name}</p>
          <p>{item.category}</p>
